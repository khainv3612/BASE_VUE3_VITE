<template>
	<div class="otp_component">
		<div class="otp otp-mobile">
			...
			>
				<el-form-item
					class="lst_input_code_div"
					:label="$t('otp.code')"
					prop=""
				>
					<el-input
						v-for="index in lengthCode"
						:ref="lstCode[Number(index - 1)]"
						:key="index"
						...
						inputmode="numeric"
						:class="{
							'mg-3': index !== 1 && index !== lengthCode,
							'mg-r-3': index === 1,
							'mg-l-3': index === lengthCode,
						}"
						class="input-code input-code-mobile"
						maxlength="1"
				...
				<div class="resend_div">
					<div style="text-align: left" class="code-sent-email">
						<span class="notify-sent">{{
							$t('otp.notifySent', { email: hideEmail })
						}}</span>
					</div>
					<span class="resend">
						<span>{{ $t('otp.notReceived') }}</span>
						<span v-if="!isExpire">{{ $t('otp.reqCode1') }}</span
						><span v-if="!isExpire" class="color-field">{{
							$t('otp.reqCode2', { time: countTime })
						}}</span>
						<span
							v-else-if="!isResendOtp"
							:class="{ noselect: isResendOtp }"
							class="color-field cursor-pointer"
							@click="resendOtp"
							>{{ $t('otp.resendCode') }}</span
						>
					</span>
				</div>
...
<script setup>
	...
	const disabledButton = computed(() => {
		...
		for (let i = 0; i < verifyCode.value.length; i++) {
			if (
				verifyCode.value[i] == null ||
				!verifyCode.value[i].toString().trim()
			) {
				return true
			}
		}
	...
	const hideEmail = computed(() => {
		...
		let count = 0
		return emailTemp.replace(/(.{2})(.*)(?=@)/, function (gp1, gp2, gp3) {
