		...
		<div v-if="page === 'login'">
			...
			>
				<div class="login">
					...
					<el-form-item prop="captcha" :error="errorResponse.captcha">
						<vue-recaptcha
							ref="captcha"
							:sitekey="sitekey"
							@error="onError"
					...
					<div
						class="flex items-center justify-end color-green mb-3 font-medium"
					>
	...
	const handleForgot = (ruleForgotFormRef) => {
		...
		ruleForgotFormRef.validate(async (valid) => {
			if (valid) {
				loadingService.startLoading()
				try {
					const response = await authService.handleForgotPass(
						forgotForm.value
					)
					if (response.status_code === 200) {
