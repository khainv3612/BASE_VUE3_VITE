<template>
      ...
      >
        <div class='login'>
          ...
          <BaseInputConfirm
            ...
            v-model:passwordConfirmation="registerForm.password_confirmation"
            :password="registerForm.password"
            :password-confirmation="registerForm.password_confirmation"
            :rule-form-ref="ruleFormRef"
          ...
          />
          <el-form-item
            prop='captcha' :error="errorResponse['g-recaptcha-response']"
          >
            <vue-recaptcha
              ref='captcha'
              :sitekey='sitekey'
          ...
          >
            ...
            <span class='cursor-pointer color-green' @click="router.push('/login')">{{
                $t('common.log_in')
              }}</span>
          </div>
        </div>
  ...
  <otp-component v-if='step===2' :token='token' :user='userForm'></otp-component>
</template>
    ...
    if (valid) {
      ...
      try {
        ...
        if (response.status_code === 200) {
          ...
          userForm.value = { ...registerForm, 'g-recaptcha-response': captchaStr.value }
          step.value = 2
          loadingService.stopLoading()
          await notiService.setNotifySuccess(trans('noti.register_success'))
